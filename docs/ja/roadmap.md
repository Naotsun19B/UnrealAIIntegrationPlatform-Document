...
### 人間向けエディタ GUI
AI の活動を人間がモニタリングするためのオプショナルエディタタブ。CommandHistory（コマンドとレスポンスのタイムライン）・ArtifactViewer（スクリーンショット・JSON ダンプ・レポートのインラインプレビュー）・ScenarioBuilder（JSON を手書きせずにシナリオを組み立てて実行できる UI）を想定しています。
