...
> 動くサンプル JSON が必要な場合は [Cookbook](cookbook.md) に直接ジャンプしてください。本ページは「なぜそれをやりたくなるか」の文脈整理に重点を置いています。
...
## 4. エディタ側の AI ペアプログラミング

**こんな人向け**：Claude Code / Cursor / Copilot をメイン IDE として使っているソロ開発者や小規模チーム。
...
- **信頼できない公開**：UAIP は localhost 専用で、ホストユーザーを信頼することを前提に設計されています。詳細は [セキュリティ → 脅威モデル](security.md#脅威モデル) を参照してください
