...
`[UAIP.SafetyPolicy]` 以外にも、UAIP は次の ini セクションを `Config/DefaultUAIP.ini` から読み込みます。いずれもデフォルト値で動作するため、必要な場合にのみ上書きしてください。各キーは `-uaip-...` 形式の CLI フラグからも上書きできます。

### `[UAIP.Session]` — セッション

セッションフォルダとライフサイクルを制御します。

| キー | 既定値 | 内容 |
|---|---|---|
| `Enabled` | `True` | セッション機構を有効化 |
| `SubDirectory` | （内部既定値） | `Saved/UAIP/` 配下のサブディレクトリ名 |
| `MaxCommandLogEntries` | （内部既定値） | 1 セッションが保持するコマンドログの最大件数 |
| `SessionLifetimeHours` | （内部既定値） | セッションを生存させる時間（時間単位） |
| `MaxAllowedLifetimeHours` | （内部既定値） | 設定可能なライフタイムの上限 |
| `MaxScanFiles` | （内部既定値） | 起動時のセッションスキャンで読む最大ファイル数 |

CLI 上書き：`-uaip-session-enabled` / `-uaip-session-sub-directory` / `-uaip-session-max-command-log-entries` / `-uaip-session-lifetime-hours` / `-uaip-session-max-allowed-lifetime-hours` / `-uaip-session-max-scan-files`。

### `[UAIP.ArtifactGC]` — Artifact のガベージコレクション

`Saved/UAIP/` 配下に蓄積された Artifact を時間・件数で整理します。

| キー | 既定値 | 内容 |
|---|---|---|
| `Enabled` | `True` | Artifact GC を有効化 |
| `MaxAgeHours` | （内部既定値） | この時間を超えた Artifact を削除対象に |
| `MaxSessionCount` | （内部既定値） | セッション数の上限（古いセッションから削除） |
| `CleanupIntervalSeconds` | （内部既定値） | GC ループを回す間隔（秒） |

CLI 上書き：`-uaip-gc-enabled` / `-uaip-gc-max-age-hours` / `-uaip-gc-max-session-count` / `-uaip-gc-cleanup-interval-seconds`。

### `[UAIP.CommandNotification]` — Editor 通知

コマンドの成否を Slate 通知として右下に表示する挙動を制御します。

| キー | 既定値 | 内容 |
|---|---|---|
| `Enabled` | `True` | Slate 通知の表示を有効化 |
| `DurationSeconds` | `1.0`〜`30.0` の範囲 | 通知の表示時間 |
| `MaxConcurrentNotifications` | `1`〜`20` の範囲 | 同時に表示する通知の最大件数 |
| `ThrottleWindowSeconds` | `0.1`〜`60.0` の範囲 | 同種通知のスロットリング窓 |

### `[UAIP.PythonExtension]` — Python コマンド拡張（🧩 `PythonScriptPlugin`）

`@uaip_command` で登録される Python コマンドのスキャン対象を指定します。

| キー | 既定値 | 内容 |
|---|---|---|
| `CommandsDir` | （プロジェクトの Content 配下のデフォルト） | Python コマンドの定義ファイルを探すディレクトリ |

> 上記の「既定値」のうち「内部既定値」と書いた箇所は、実装にハードコードされた値に従います。利用方法を変えたい場合のみ ini で上書きしてください。
