...
UAIP デモ版は GitHub Releases で無償配布する機能制限付きバイナリです。観測・PIE 制御・アサーション・シナリオ実行・UI 自動化コマンドを提供しており、無償で AI エージェントをレビューやテストのワークフローに組み込めます。
...
| Canonical グラフキャプチャ（GraphPrinter 連携） | — | ✅ |
| **その他** | | |
| キャプチャ画像への透かし | ✅ | — |
...
`Config/DefaultUAIP.ini` を Pro 版のものに置き換えるだけです。プラグインフォルダの構造と MCP サーバー登録は同一なので、それ以外の変更は不要です。
...
> `CaptureCanonicalGraphImage`（GraphPrinter 連携）はデモ版では**利用不可**です（`CommandNotFound` を返します）。
...
Pro 版には存在するが、デモ版では `CommandNotFound` を返すコマンド：

| コマンド | 理由 |
|---|---|
| `UAIP.Editor.Observation.CaptureCanonicalGraphImage` | GraphPrinter 連携は Pro 版のみ |
| `UAIP.Editor.Execution.RunEditorPythonScript` | Python 実行は Pro 版のみ |
| ホワイトリスト外モジュールが提供する全コマンド | Editor 編集・Runtime ワールド編集・GAS・Niagara・Input inject など |

### ロードフェーズ

デモ版バイナリはエディタ専用（`EditorNoCommandlet`）でロードされます。パッケージビルドやコマンドレット実行環境ではロードされません。外部プラグイン依存の宣言がないため、プロジェクトに不要なプラグインエントリが表示されません。
