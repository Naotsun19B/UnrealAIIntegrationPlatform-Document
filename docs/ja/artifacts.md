...
PNG をインライン化するかどうかは利用ポリシー次第です。デフォルトでは画像インラインが OFF になっています（PNG をセッション全体で蓄積すると AI クライアントのトークン予算を圧迫しやすいためです）。`config.json` の `inline_artifacts.image` を `true` に変えれば変更できます。
