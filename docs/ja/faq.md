...
- **AI を主軸に開発する人** — Claude Code / Cursor / Windsurf / Copilot 上のセッションから、実際にエディタを動かしたい（アセットを開く、Blueprint を編集する、PIE を実行する、スクリーンショットを撮る、など）
...
Python は実行チャネルのひとつにすぎません。UAIP は **意味レベルのコマンド** を、宣言済みのパラメータスキーマ・必要な Capability・統一された Artifact 出力とセットで、MCP・HTTP・WebSocket・CLI の 4 つの transport から公開します。Python スクリプト（`RunEditorPythonScript`）も使えますが、Capability や Policy のレイヤーをバイパスしてしまうため、登録済みコマンドでまかなえないケースに限定して使うのが原則です。
...
右下に `UAIP Demo` のテキストとプラグインアイコンのバナーをアルファブレンドで合成しています。透かしのデータは DLL にコンパイル済みなので、ファイルを差し替えても除去できません。万が一透かしの合成に失敗したときは、透かしなしの画像を保存せずに `ExecutionFailed` を返す（フェイルクローズ）仕様です。
...
デモバイナリは Windows 専用です。製品版は MCP Bridge と CLI / HTTP / WS トランスポートが macOS / Linux にも対応しますが、グラフエディタ系コマンドや一部のオプションプラグインは Windows での検証が先行しています。
...
詳細仕様は [シナリオ実行](scenario.md) を、テンプレートの実例は [Cookbook](cookbook.md) を参照してください。
...
ユースケースを Issue にまとめてご投稿ください。候補は [ロードマップ](roadmap.md) で管理しています。横断的な設計変更には ADR が必要ですが、小規模な追加は既存モジュールのパターンに沿って実装できます。
