...
はい、同じエディタへ複数セッションを接続すること自体は可能です。ただし既定では同時に実行できるコマンドは 1 件だけです。この上限を課しているのは **transport 側**（in-flight のコマンド枠が 1 本しか無い）であって、ディスパッチャ自体が直列化しているわけではありません。あるセッションのコマンドが実行中の間、他セッションのコマンドはキューに積まれるのではなく `TooManyRequests` で**その場で断られ**、呼び出し側が再送する前提になっています。シナリオの同時投入も同じ扱いで、実行できるシナリオは常に 1 件だけです。

この単一枠の制約が特に痛いのは、コマンドが何も変更せず「ただ待つだけ」の場合です — 人間の操作待ちやシェーダーコンパイル待ちをしているセッションが、その間ずっと枠を握り続け、他の全セッションを数分単位で締め出してしまいます。`Config/DefaultUAIP.ini` に `[UAIP.Transport].AllowConcurrentPassiveWaits=True` を設定すると（HTTP / MCP のみ対象）、こうした特定の待機コマンドだけを枠の占有から外せます — 詳細は [設定リファレンス → `[UAIP.Transport]` 受動的待機の同時実行](config.md#uaiptransport--受動的待機の同時実行既定オフ) を参照してください。状態を変更するコマンドはこの設定に関わらず引き続き 1 件ずつしか実行されません — 同じエディタへの並行編集は壊れ方が非決定的になるため、これは対象外のままです。

本当に並行して**編集**作業をしたい場合（並行して待つだけではなく）は、この設定では実現できないため、エディタやプロジェクトを分けてください。
