...
> MCP Bridge（`Tools/MCPBridge/`）には独自の設定レイヤー（`config.json` + 環境変数）があります。下の [MCP Bridge `config.json`](#mcp-bridge-configjson) を参照。
...
MCP Bridge（`Tools/MCPBridge/`）経由で接続する場合、追加の JSON 設定レイヤーが適用されます。これは Python プロキシ側が読み込むもので、エディタ側は関与しません。
...
環境変数が設定されている場合は JSON 値を上書きします。フル コメント付きテンプレートは `Tools/MCPBridge/config.json.example`（プラグイン同梱）を参照してください。
