...
- **Chaos Cloth Asset 編集**（`UAIPEditorChaosClothAsset` モジュール、**新規**、`ChaosClothAsset` プラグインファミリー必須、UE 5.8、Experimental）: AI エージェントが Clothing Asset の Skeletal Mesh セクションへのバインド/アンバインド、Chaos Cloth Asset の作成・変換、Weight Map 頂点データの読み書き、Import Dataflow ノードへのインポート元メッシュ参照設定を行えるようになりました。`UAIP.Editor.ChaosClothAsset` 配下に9コマンドを追加しました — `CreateClothingAsset`、`AssignClothingToSection`、`RemoveClothingFromSection`（破壊的・不可逆）、`ListClothingAssets`、`GetSectionClothing`、`ConvertClothingAssetCommonToChaosClothAsset`（Experimental、LOD0のみ）、および Toolset に対応がない native 専用3コマンド：`GetClothAssetInfo`（LOD数・Sim/Render Meshの頂点数・参照先 `UDataflow` アセットパス・Weight Map 属性名を取得）、`SetClothWeightMapVertexValues`（Weight Map ノードの頂点ごとの重み配列を直接書き込む破壊的操作）、`SetClothMeshImportSource`（Import ノード `SkeletalMeshImport`/`StaticMeshImport` へのインポート元 SkeletalMesh/StaticMesh 参照設定；ノード種別は自動判定、既存参照の上書きには `AllowOverwrite` が必要な破壊的操作）。新規 DefaultDenied Capability を1つ追加：`ClothAssetEdit`。UE 5.8+ で `ChaosClothAssetToolset` プラグインが有効な場合、同じ操作をミラーする6つのブリッジコマンドが `Toolset.Editor.ChaosClothAsset.*` 配下に追加で利用可能です。
...
- **Physics編集ユニットテストをディスク上に存在しないテスト用アセットへの依存から解放**（`UAIPEditorPhysics` モジュール、内部改善）: 本モジュールの AutomationTest がプラグインリポジトリに一度もコミットされていない `/Game/Tests/Physics/SK_TestRagdoll_Physics` アセットを参照しており、Handlerレベルのテスト18件が `NotFound` で失敗していました。30個のネイティブコマンド（`CreatePhysicsAsset` を除く全て。これは `PhysicsAsset` ではなく `SkeletalMesh` をロードするため対象外）のアセットロードは、ControlRig / StateTree / ChaosClothAsset / EQS / WorldConditions で既に採用されているのと同じパターンで、注入可能な `IPhysicsAssetLoader` インターフェース経由に変更されました。コマンドの外部パラメータ・エラーコード・Capability に変更はありません。
