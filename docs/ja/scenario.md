...
不正な参照（未知のサブ識別子、空のポインタ、不正な `~` エスケープ、ステップのデータに一致しないポインタ、サイズ超過の値、混在文字列へのオブジェクト・配列埋め込みなど）は、`ErrorCode: InvalidParams` でステップを失敗させます。これは**リトライされません**：`RetryCount` は `ExecutionFailed` と `PreconditionFailed` にのみ適用されます。
...
シナリオ内の編集コマンドは、成功して実際に変更を記録するたびに、取り消し履歴（Undo History）へ 1 件ずつ独立して積まれます。

### 導入前後の違い

- **以前**: シナリオに編集コマンドを複数並べても、取り消し履歴に残るのは最初に成功した編集コマンドの名前 1 件だけでした。それを 1 回取り消すと、名前とは無関係にシナリオ中のすべての編集がまとめて巻き戻っていました。
- **現在**: シナリオ内の編集コマンドが 1 つずつ取り消し履歴に積まれます。履歴の件数はシナリオのステップ数ではなく、**実際に変更を記録したコマンドの件数**になります。読み取り専用のステップは履歴を積みません。

これにより、`Undo` の応答が返す名前と、実際に戻る範囲が一致するようになりました。直前の 1 コマンドだけを戻すことも、`StepCount` を指定してシナリオの編集をまとめて戻すこともできます。詳細は [コマンドリファレンス](commands.md) の `Undo` / `Redo` を参照してください。

### `StepCount` の上限

`Undo` / `Redo` の `StepCount` パラメータの上限は **100** です（シナリオの [上限値](#上限値) の最大ステップ数と一致しています）。上限を揃えているのは、per-step 粒度で積まれた長いシナリオでも、`Undo` を 1 回呼べば全体を戻せるようにするためです。

上限を揃えても「1 回で必ずシナリオ全体が戻る」わけではありません。シナリオの後に別の編集が挟まっていないこと、各編集ステップがちょうど 1 件を履歴に積んでいること、その全件がまだ履歴に残っていることが前提です。

### 注意点

- 取り消しはエディタ唯一の取り消し履歴に対して働き、**呼び出し元を区別しません**。並行して作業している別のセッションや、人間による手作業の編集も同じ履歴に並びます。大きな `StepCount` を指定すると、それらを巻き込んで戻る範囲も広がります。
- **`GetUndoHistory` で、実行する前に何が戻るかを確認できます。** 何も実行せずに、戻せる件数・やり直せる件数と操作名を返します。取り消しの権限（`EditorUndoRedo`、既定では禁止）を必要とせず `EditorInspect` だけで呼べるため、取り消しが許可されていない環境でも履歴を読めます。
- 大きな `StepCount` を指定する前にこれで対象を確認してください。刻んで呼び出しながら応答の名前を見る方法も引き続き使えます。
