...
**MCP Bridge**（本リポジトリの Releases で配布する `UAIP-MCPBridge-<version>.zip`）は [MIT License](https://opensource.org/licenses/MIT) のもとで提供されます。ライセンス全文はアーカイブ内に `LICENSE` として同梱されています。  
