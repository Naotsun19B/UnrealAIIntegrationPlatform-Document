...
本リポジトリの [Releases](../../../releases?q=Demo)（`Demo-v<X.Y.Z>` タグ）で配布するデモバイナリは、リリースアーカイブに同梱の `EULA.txt` に従って提供されます。  
Fab で配布される製品版は [Fab Standard License (Fab EULA)](https://www.fab.com/ja/eula) に基づいて提供されます。  
**MCP Bridge**（本リポジトリの [Releases](../../../releases?q=MCPBridge) で `MCPBridge-v<X.Y.Z>` タグとして配布する `UAIP-MCPBridge-<version>.zip`）は [MIT License](https://opensource.org/licenses/MIT) のもとで提供されます。ライセンス全文はアーカイブ内に `LICENSE` として同梱されています。  
