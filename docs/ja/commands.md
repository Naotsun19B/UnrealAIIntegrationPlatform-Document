...
| 記号 | 意味 |
|---|---|
| 🆓 | デモバイナリで利用可能（Pro 版でも利用可能） |
| (記号なし) | Pro 版限定コマンド |
| **†** | オプションプラグインが必要（プラグインが無効の場合は登録されません） |

## Toolset ブリッジ（UE 5.8+）

UAIP ネイティブコマンドに加えて、Pro 版では公式 UE 5.8 Toolset フレームワークに委譲する **Toolset ブリッジコマンド**（`Toolset.*` プレフィックス）を提供します。これらは UAIP ネイティブコマンドの代替パスです。本ページではドメイン別にサマリのみを記載しているため、実行時に列挙するには `uaip_list_commands(ProviderPrefix="Toolset")` を使用してください。

---

## ドメインサマリ

| ドメイン | プロバイダプレフィックス | ネイティブ | Toolset ブリッジ | デモ |
|---|---|---:|---:|---:|
| Core | `UAIP.Core` | 6 | — | ✅ |
| Editor Workspace | `UAIP.Editor.Workspace` | 17 | — | 一部 |
...
エディタライフサイクル・タブ管理・グラフレイアウト・シェーダーコンパイル。
...
| `TriggerLiveCoding` | Live Coding 再コンパイルをトリガーし完了を待つ |
...
| `ListWidgetBlueprints` | AssetRegistry から Widget Blueprint 一覧（最大 500 件） |
...
GameFeature Plugin 管理。`GameFeatures` + `GameFeaturesEditor` プラグインが必要です（作成コマンドは両方必須）。
...
Niagara VFX システム編集。`Niagara` + `NiagaraEditor` プラグインが必要です。

### ネイティブ（30）

**観測**：`GetSystemTopology` / `GetSystemCompileState` / `GetAssetDiscoveryInfo` / `GetScriptAssets` / `GetNiagaraParameterCollections` / `GetUserVariables` / `GetSystemInfo` / `GetSystemData` / `GetEmitterData` / `GetRendererData` / `GetStackInputData` / `UEnum_Info`

**編集**：`AddEmitter` / `RemoveEmitter` / `DuplicateEmitter` / `SetEmitterEnabled` / `SetEmitterName` / `SetEmitterData` / `AddRenderer` / `RemoveRenderer` / `SetRendererData` / `AddModule` / `RemoveModule` / `MoveModule` / `SetModuleEnabled` / `SetStackInputData` / `SetSystemData` / `AddUserVariables` / `RemoveUserVariables` / `CompileNiagaraSystem`

注：`CreateNiagaraSystem` と `CreateNiagaraParameterCollection` は標準 `CreateAsset` コマンドで提供（個別コマンドはありません）。
...
**アセット / 観測**：`CreatePhysicsAsset` / `GetPhysicsAssetSummary` / `ValidatePhysicsAsset`

**ボディ**：`GetBodyNames` / `AddBody` / `RemoveBody` / `GetBodyPhysicsMode` / `SetBodyPhysicsMode` / `SetAllBodiesPhysicsMode` / `GetBodyMassScale` / `SetBodyMassScale` / `GetBodyCollisionProfile` / `SetBodyCollisionProfile` / `SetBodyLinearDamping` / `SetBodyAngularDamping` / `GetBodyOffset` / `SetBodyOffset` / `MirrorBodies`

**シェイプ**：`GetBodyShapes` / `SetSphere` / `SetCapsule` / `SetBox` / `RemoveShape` / `RegenerateBodyShapes` / `CopyBodyShapes` / `SetPhysicalMaterial`

**コンストレイント**：`GetConstraints` / `ListConstraintsForBody` / `AddConstraint` / `SetConstraintLimits` / `RemoveConstraint`
...
**構造（16）**：`AddTrack` / `RemoveTrack` / `AddSection` / `RemoveSection` / `SetPlaybackRange` / `FlushSequencerChanges` / `CreateLevelSequence` / `GetAvailableSequencerTrackClasses` / `SetSectionRange` / `DuplicateSection` / `MoveSection` / `AddCameraCut` / `SetTrackEnabled` / `IsTrackEnabled` / `SetSectionActive` / `IsSectionActive`

**キーフレーム（7）**：`AddKeyframe` / `RemoveKeyframe` / `SetKeyframeValue` / `SetKeyframeInterpolation` / `SetKeyframeTangents` / `OffsetKeyframes` / `GetKeyframeTangents`

**バインド（4）**：`BindActor` / `UnbindActor` / `GetActorBindingGuid` / `GetBoundActors`

**観測（12）**：`GetSequenceInfo` / `GetBindings` / `GetTracks` / `GetSections` / `GetDisplayRate` / `GetTickResolution` / `GetPlaybackRange` / `GetKeyframes` / `ValidateSequenceBindings` / `GetCameraCutSections` / `GetCurrentSequence` / `GetFocusedSequence`

**再生（10）**：`Play` / `Pause` / `IsPlaying` / `SetPlayheadFrame` / `GetPlayheadFrame` / `SetPlaybackSpeed` / `GetPlaybackSpeed` / `SetLoopMode` / `GetLoopMode` / `ForceEvaluate`

**セクションプロパティ（4）**：`GetSectionProperty` / `SetSectionProperty` / `GetSectionWeight` / `SetSectionWeight`

**UI / 状態（10）**：`SetCameraLock` / `IsCameraLockActive` / `GetSelectionRange` / `SetSelectionRange` / `ClearSelection` / `GetTrackFilterNames` / `IsTrackFilterActive` / `SetTrackFilterActive` / `SetLocked` / `IsLocked`

**シーケンスプロパティ（6）**：`SetDisplayRate` / `GetViewRange` / `SetViewRange` / `GetWorkRange` / `SetWorkRange` / `SetTickResolution`

**マークフレーム（5）**：`AddMarkedFrame` / `GetMarkedFrames` / `DeleteMarkedFrame` / `DeleteAllMarkedFrames` / `FindMarkedFrameByLabel`

**サブシーケンス（2）**：`GetSubSequences` / `AddSubSequenceTrack`

**AnimMixer（17、オプショナル `MovieSceneAnimMixer`）**：`GetAnimMixerTrackInfo` / `GetLayerBlendWeight` / `SetLayerBlendWeight` / `IsLayerMuted` / `SetLayerMuted` / `IsLayerEnabled` / `SetLayerEnabled` / `ClearMixerLayer` / `AddMixerLayer` / `RemoveMixerLayer` / `MoveMixerLayer` / `AddMixerSection` / `RemoveMixerSection` / `SetMixerSectionRange` / `SetMixerSectionAnimation` / `AddMixerTransition` / `RemoveMixerTransition` / `GetMixerSectionInfo`
...
**ヒエラルキー観測（10）**：`GetElements` / `GetAllBones` / `GetAllNulls` / `GetAllControls` / `GetGlobalTransform` / `GetLocalTransform` / `GetParent` / `GetChildren` / `GetModuleInstances` / `GetControlSettings`

**ヒエラルキー編集（11）**：`AddElement` / `AddBone` / `AddNull` / `AddControl` / `RemoveElement` / `RemoveBone` / `RemoveNull` / `RemoveControl` / `ReparentElement` / `SetControlOffset` / `SetControlSettings`

**トランスフォーム（3）**：`SetGlobalTransform` / `SetLocalTransform` / `ImportBonesFromAsset`

**グラフ管理（11）**：`ListGraphs` / `GetGraph` / `AddGraph` / `DeleteGraph` / `GetForwardSolveGraph` / `GetBackwardSolveGraph` / `GetInteractionGraph` / `GetEventGraph` / `AddEventGraph` / `AddBackwardSolveGraph` / `AddInteractionGraph`

**ノード（10）**：`AddGraphNode` / `RemoveGraphNode` / `ListNodes` / `GetNodeInfo` / `FindNodes` / `GetNodePosition` / `SetNodePosition` / `DuplicateNode` / `AddEventNode` / `AddVariableNode`

**ピン（7）**：`ListPins` / `GetPinValue` / `SetPinValue` / `ResetPinValue` / `GetConnectedPins` / `ConnectControlRigPins` / `DisconnectControlRigPins`

**変数（5）**：`AddVariable` / `ListVariables` / `GetVariable` / `ChangeVariableType` / `RemoveVariable`

**その他（2）**：`CompileControlRig` / `GetAvailableRigVMUnitStructs`

ControlRigBlueprint の作成は標準 `CreateAsset` コマンドで提供されます（`ControlRigBlueprintCreate` Capability 必須）。
