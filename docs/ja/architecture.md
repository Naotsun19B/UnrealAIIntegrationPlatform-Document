...
大原則として、**依存方向は下位レイヤーへの一方向のみ** です。Transport レイヤーは Domain ハンドラをインポートしませんし、Domain ハンドラも Transport をインポートしません。Core が両者の中間に立っています。
...
セッションは次のものを所有する単位です：
- コマンドログ（永続化される。`MaxCommandLogEntries` の上限を受ける）
- セッション内のコマンド間で共有されるキーバリューのコンテキスト
- 役割を運ぶリクエストが解決した後の、束縛済み役割名（未束縛なら空 — 詳細は [Safety & Capabilities → 役割](safety.md#役割layer-15)）
- `SessionId` を単位とする Artifact のサブフォルダ（`Saved/UAIP/<SessionId>/`）

セッションは自身の Capability セットをキャッシュとして持ち**ません**。dispatch と discovery が使う実効セットはその都度計算されます — プロセス全体の集合（Layer 1）を、束縛済みの役割があればその役割で狭めたもの（Layer 1.5）です。そのため `ReloadCapabilities` の呼び出しや新規の役割束縛は、セッション側の何かを無効化する必要なく即座に反映されます。同様に、シナリオ送信などのルートに対するレート制限もプロセス全体のゲートであり（エディタ全体で同時に走れるシナリオは 1 本のみ）、セッションごとのカウンタではありません。

匿名セッション（`SessionId` を指定しない場合）には ID が自動付与されます（MCP トランスポートなら `MCP-Anonymous-<guid>`）。単発の呼び出しには便利ですが、タスクごとにセッションを分けたほうが Artifact を後から探しやすくなります。なお、後から照会するための識別子を返すコマンドは匿名セッションを拒否します。匿名セッションは呼び出しごとに別物が作られるため、返した識別子を二度と照会できなくなるからです（アセット監査のジョブ系コマンドとトレース解析系コマンドは、いずれも `SessionId` の明示指定が必須です）。

---

## 7. Artifact ライフサイクル

出力を生成するコマンド（キャプチャ・ダンプ・ログ・レポート）はすべて、1 つ以上の **Artifact** を `Saved/UAIP/<SessionId>/` に書き出し、レスポンスには Artifact ID を返します。クライアントは ID を介して内容を取得する仕組みで、ファイルパス自体はレスポンスペイロードに含めません。これによりパスリーク攻撃を防ぎつつ、どのトランスポートからアクセスしても同じ取り扱いに統一できます。
