...
|---|---|---|---|
| HTTP transport（Pro） | `127.0.0.1` と `::1` | 8765（エディタ）/ 8767（パッケージ） | Bearer トークン |
| WebSocket transport（Pro） | `127.0.0.1` と `::1` | 8766（エディタ）/ 8768（パッケージ） | Bearer トークン（最初のフレーム） |
| MCP Bridge | AI クライアントと Bridge プロセス間の stdio | — | なし — ホスト信頼に依存 |
| CLI transport（Pro） | なし（プロセス内） | — | なし |
