...
- **`UAIP.Editor.Engine.Log.GetLogVerbosity` no longer returns garbled output**: The previous implementation used `FStringOutputDevice`, which prepended event-tag prefixes to each line and caused the verbosity parser to fail. Replaced with a custom `FOutputDevice` subclass that suppresses those prefixes.
