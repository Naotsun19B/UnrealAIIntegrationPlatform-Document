...
Beyond `[UAIP.SafetyPolicy]`, UAIP reads the following sections from `Config/DefaultUAIP.ini`. All of them work with sensible defaults — only override what you need. Each key can also be overridden by the matching `-uaip-...` CLI flag at editor launch.

### `[UAIP.Session]` — sessions

Controls the session folder layout and lifetime.

| Key | Default | Meaning |
|---|---|---|
| `Enabled` | `True` | Enable the session subsystem |
| `SubDirectory` | (internal default) | Subdirectory name under `Saved/UAIP/` |
| `MaxCommandLogEntries` | (internal default) | Maximum command-log entries kept per session |
| `SessionLifetimeHours` | (internal default) | How many hours a session stays alive |
| `MaxAllowedLifetimeHours` | (internal default) | Upper bound on the lifetime users can configure |
| `MaxScanFiles` | (internal default) | Maximum session files scanned at startup |

CLI overrides: `-uaip-session-enabled` / `-uaip-session-sub-directory` / `-uaip-session-max-command-log-entries` / `-uaip-session-lifetime-hours` / `-uaip-session-max-allowed-lifetime-hours` / `-uaip-session-max-scan-files`.

### `[UAIP.ArtifactGC]` — artifact garbage collection

Reaps artifacts under `Saved/UAIP/` by age and session count.

| Key | Default | Meaning |
|---|---|---|
| `Enabled` | `True` | Enable artifact GC |
| `MaxAgeHours` | (internal default) | Artifacts older than this become removal candidates |
| `MaxSessionCount` | (internal default) | Upper bound on retained session count (oldest go first) |
| `CleanupIntervalSeconds` | (internal default) | GC loop interval in seconds |

CLI overrides: `-uaip-gc-enabled` / `-uaip-gc-max-age-hours` / `-uaip-gc-max-session-count` / `-uaip-gc-cleanup-interval-seconds`.

### `[UAIP.CommandNotification]` — editor toast notifications

Controls the per-command success / failure Slate notification shown in the editor.

| Key | Default | Meaning |
|---|---|---|
| `Enabled` | `True` | Show command notifications |
| `DurationSeconds` | range `1.0`–`30.0` | How long each notification stays visible |
| `MaxConcurrentNotifications` | range `1`–`20` | Maximum simultaneous notifications |
| `ThrottleWindowSeconds` | range `0.1`–`60.0` | Throttle window for repeating notifications |

### `[UAIP.PythonExtension]` — Python command extension (🧩 `PythonScriptPlugin`)

Selects where the Python command scanner looks for `@uaip_command` definitions.

| Key | Default | Meaning |
|---|---|---|
| `CommandsDir` | (project Content default) | Directory containing Python command definition files |

> Entries marked "internal default" use the value hard-coded in the implementation. Override them via the ini only if you specifically need different behavior.
