...
### MCP Bridge
The thin Python proxy (`Plugins/UAIPMCPBridge/thin_proxy.py`) that connects an AI client to the UE Editor. Translates MCP tool calls into UAIP HTTP requests internally, manages editor lifecycle (auto-launch, crash/hang recovery), and handles artifact inlining. Distributed separately from the plugin as `UAIP-MCPBridge-<version>.zip` (under `MCPBridge-v<X.Y.Z>` tags) in the documentation repository's [Releases](../../releases?q=MCPBridge).
