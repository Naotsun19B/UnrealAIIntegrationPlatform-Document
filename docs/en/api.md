...
| `OptionalPluginDisabled` | Depends on an optional plugin that was disabled when this process was **built**, so the types the command needs were compiled out of the binary entirely | Enabling the plugin and **rebuilding** — restarting alone does not help, since the code path doesn't exist in this binary |
